...
const ExportProgressPopup = ({ onClose, data, courseName }) => {
    ...
    const exportToCSV = (progressData) => {
        if (!progressData || progressData.length === 0) return;

        // Define headers
        const headers = ['Name', 'User Email', 'Course Name', 'Start Date', 'Progress %', 'Status'];
        ...
        link.setAttribute('download', `Progress_Export_${courseName || 'Course'}_${new Date().toLocaleDateString()}.csv`);
    ...
    const exportToPDF = (progressData) => {
        if (!progressData || progressData.length === 0) return;
        ...
        doc.setFontSize(14);
        doc.setTextColor(50);
        doc.text(`Course: ${progressData[0]?.courseName || courseName || 'N/A'}`, 14, 30);
        ...
        // Save PDF
        doc.save(`Progress_Export_${date}.pdf`);
    };
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-[9999] backdrop-blur-sm">
            ...
            >
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                            <input
                                ...
                                className="sr-only"
                            />
                            <div className={`w-6 h-6 rounded-full border-2 border-white flex items-center justify-center transition-all ${exportFormat === 'CSV' ? 'bg-[#FF9D00]' : 'bg-transparent'}`}>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                            <input
                                ...
                                className="sr-only"
                            />
                            <div className={`w-6 h-6 rounded-full border-2 border-white flex items-center justify-center transition-all ${exportFormat === 'PDF' ? 'bg-[#FF9D00]' : 'bg-transparent'}`}>
                ...
                <div className="flex items-center gap-4 mb-10 cursor-pointer self-start ml-2 group" onClick={() => setIsAgreed(!isAgreed)}>
                    <div className={`w-6 h-6 rounded-md flex items-center justify-center border-2 transition-all duration-300 ${isAgreed ? 'bg-[#FF9D00] border-white' : 'bg-transparent border-white/50 group-hover:border-white'}`}>
...
export default ExportProgressPopup;
